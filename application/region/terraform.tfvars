application_aws_account_number = "123456" # AWS application account number (without hyphens)
...
iam_cross_account_role_arn = "arn:aws:iam::123456:role/CallRole" # Cross-account role to assume before deploying the cluster

k8s_master_instance_type = "m4.large" # Instance type (size) for master nodes

k8s_masters_iam_policies_arns = [
  "arn:aws:iam::123456:policy/masters.demo-test.k8s.local",
  "arn:aws:iam::123456:policy/masters_extra.demo-test.k8s.local",
...
k8s_nodes_iam_policies_arns = [
  "arn:aws:iam::123456:policy/nodes.demo-test.k8s.local",
] # List of existing IAM policies that will be attached to instance profile for worker nodes (EC2 instances)

k8s_private_subnets = ["subnet-12345}", "subnet-23456", "subnet-34567"] # List of private subnets (matching AZs) where to deploy the cluster (required if existing VPC is used)
...
vpc_id = "vpc-12345" # ID of existing VPC (must be pre-created)